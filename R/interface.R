...
#' @param facilities Optional character vector of facility level column names (perfect -> minimal).
...
vrcm <- function(
  ...
  out <- vrc_fit(
    ...
    mortality_facilities = mortality$facilities,
    reporting_facilities = reporting$facilities,
...
vrc_coef_summary <- function(
  ...
  if (!inherits(x, "vrcfit")) {
    stop("x must be a vrcfit object")
  }

  K_mort <- x$standata$K_mort
  K_rep <- x$standata$K_rep
  K_fac_mort <- x$standata$K_fac_mort
  K_fac_rep <- x$standata$K_fac_rep
  ...
  if (K_fac_mort > 0) {
    pars <- c(pars, "beta_fac_mort_pars")
  }
  if (K_fac_rep > 0) {
    pars <- c(pars, "gamma_fac_rep_pars")
  }
  ...
  if (!is.null(e$beta_conf)) {
    mat <- if (length(dim(e$beta_conf)) == 1) {
      matrix(e$beta_conf, ncol = 1)
    } else {
      e$beta_conf
    }
  ...
  if (K_fac_mort > 0 && !is.null(e$beta_fac_mort_pars)) {
    terms <- x$meta$model_options$mortality_facilities
    for (g in seq_len(dim(e$beta_fac_mort_pars)[3])) {
      mat <- matrix(e$beta_fac_mort_pars[,, g], ncol = K_fac_mort)
      sm <- summarise_mat(mat)
      rows[[length(rows) + 1]] <- data.frame(
        component = "mortality",
        parameter = "beta_fac",
        cause = rep(cause_levels[g], K_fac_mort),
        term = terms,
        sm
      )
    }
  ...
  if (!is.null(e$gamma_conf)) {
    mat <- if (length(dim(e$gamma_conf)) == 1) {
      matrix(e$gamma_conf, ncol = 1)
    } else {
      e$gamma_conf
    }
    sm <- summarise_mat(mat)
  ...
  if (K_fac_rep > 0 && !is.null(e$gamma_fac_rep_pars)) {
    terms <- x$meta$model_options$reporting_facilities
    for (g in seq_len(dim(e$gamma_fac_rep_pars)[3])) {
      mat <- matrix(e$gamma_fac_rep_pars[,, g], ncol = K_fac_rep)
      sm <- summarise_mat(mat)
      rows[[length(rows) + 1]] <- data.frame(
        component = "reporting",
        parameter = "gamma_fac",
        cause = rep(cause_levels[g], K_fac_rep),
        term = terms,
        sm
      )
    }
